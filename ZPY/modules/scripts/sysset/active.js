define(function (require, exports, module) {
            ...
            $("#acttype").dropdown({
                ...
                onChange: function (data) {
                    ...
                    if (data.value == 1) {
                        ...
                        $('#iptCreateTime').val(_self.saveParams.BTime + "至" + _self.saveParams.ETime); 
                    } else {
    ...
    ObjectJS.bindActiveAdd = function (model) {
        ...
        $("#iptCreateTime").daterangepicker({
            ...
            ObjectJS.saveParams.BTime = start ? start.format("YYYY-MM-DD") : "";
            ObjectJS.saveParams.ETime = end ? end.format("YYYY-MM-DD") : ""; 
        }); 
        ImgsIco= Upload.createUpload({
            element: '#actimg1',
            buttonText: "选择信息海报",
        ...
        _self.saveParams.BTime = convertdate(model.BTime, true);
        _self.saveParams.ETime = convertdate(model.ETime, true); 
        $("#iptCreateTime").val(_self.saveParams.BTime + ' 至 ' + _self.saveParams.ETime);
        $("#btnSaveActive").on("click", function () {
            if (!VerifyObject.isPass()) {
                return;
        ...
        //下拉状态、类型查询
        _self.saveParams.Type = model.Type;
        require.async("dropdown", function () { 
            $("#acttype").dropdown({
                prevText: "信息类型-",
                defaultText: model.Type == 1 ? "公告" : "活动",
    ...
    ObjectJS.SaveActive = function () {
        ...
        if (_self.saveParams.Type == 1 && $('#imgurl').val()=="" ) {
            alert("信息图片未选择.");
            return false;
        }
    ...
    ObjectJS.bindDetail = function(model) {
        var _self = this;
        _self.AutoID = model.AutoID;
        $("#acttitle").val(model.Title);
        $("#acttips").val(model.Tips);
        $("#imgurl").val(model.Img);
        $("#productImg").attr("src", model.Img);
